    ...
    var body: some View {
        VStack(spacing: 35){
            ...
            VStack(spacing: 20){
                ...
                ForEach(trackViewModel.trackDatas) { trackInfo in
                    if isSameDay(date1: trackInfo.startDate, date2: currentDate){
                        VStack(alignment: .leading, spacing: 10) {
//                            ForEach(trackInfo.participations, id: \.self) { participant in
                                Text("\(trackInfo.estimatedDistance)")
                                    .font(.title2.bold())
                                Text("\(trackInfo.timeTaken)")
                                    .font(.title2.bold())
    ...
    func CardView(value: DateValue)-> some View {
        ...
        VStack{
            ...
            if value.day != -1{
                ...
                if let task = trackViewModel.trackDatas.first(where: { task in
                    
                    return isSameDay(date1: task.startDate, date2: value.date)
                }){
                    ...
                    Circle()
                        .fill(isSameDay(date1: task.startDate, date2: currentDate) ? .white : .orange)
                        .frame(width: 10, height: 10)
